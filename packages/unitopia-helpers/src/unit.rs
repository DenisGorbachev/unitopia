...
pub trait Unit: Scale {
    /// See the definition of "Dimensional (physics)" in .agents/project.md
    ///
    /// In Rust, `Dimensional` is a tuple whose every element is a pair of (QuantityKind, Power) where QuantityKind is a marker struct and Power is a `typenum` integer type.
    ///
    /// ## Requirements
    ///
