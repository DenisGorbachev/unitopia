...
macro_rules! define_strict_wrapper_units {
    ($($name:ident => $unit_marker:path),+ $(,)?) => {
        $(
            define_strict_wrapper_unit!($name, $unit_marker);
            impl_strict_wrapper_unit_ops!($name);
        )+
    };
}

define_strict_wrapper_units!(
    Ampere => unitopia_marker_units::Ampere,
    Candela => unitopia_marker_units::Candela,
    GalactosidaseActivityUnit => unitopia_marker_units::GalactosidaseActivityUnit,
    Kelvin => unitopia_marker_units::Kelvin,
    Kilogram => unitopia_marker_units::Kilogram,
    Meter => unitopia_marker_units::Meter,
    Mole => unitopia_marker_units::Mole,
    PowerOfHydrogen => unitopia_marker_units::PowerOfHydrogen,
    Second => unitopia_marker_units::Second,
);
